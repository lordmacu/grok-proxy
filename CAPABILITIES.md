...
| `audio_speech` | ✅ (con sesión) | `Chat/TextToSpeech` (streaming, concatenado) vía `POST /v1/audio/speech` | **DERIVADA del APK decompilado, no medida en vivo: este RPC nunca se llamó contra el backend real.** Los tags del frame salen de `TextToSpeechRequest.java` y el parseo está fijado por tests que sustituyen el transporte. Devuelve bytes de audio crudos con el `Content-Type` correcto, no un sobre JSON. La voz por defecto sale de `Voice/ListTopVoices`, no de un id fijo que podría no existir en la cuenta. |
| `audio_transcription` | ✅ (con sesión) | `Voice/SpeechToText` (unaria) vía `POST /v1/audio/transcriptions` | **DERIVADA del APK decompilado, no medida en vivo: este RPC nunca se llamó contra el backend real.** Devuelve `{"text": ...}` o texto plano con `response_format: "text"`. Ver nota sobre `Voice/Transcribe` abajo. |
...
| `search` | ✅ (con sesión) | `Chat/AddResponse` con `disable_search` invertido, vía `web_search` en `POST /v1/chat/completions` (y en los endpoints de mensajes de conversación) | El campo nativo de grok es `disable_search`; este proxy lo invierte y lo deja **encendido por defecto**, igual que el resto de proveedores. Esto es un cambio de comportamiento respecto de versiones anteriores de este proxy, que **sí** enviaban el campo, con `disable_search: true` por defecto — o sea, sin fundamentar. Ver `main.resolve_disable_search`. |
...
| `conversations` | ✅ (con sesión) | `alias de backend.list_conversations` / `backend.get_conversation` vía `GET /v1/conversations` y `GET /v1/conversations/{id}` | **Alias sobre RPCs que ya usaba este proxy, pero la superficie estándar nunca se ejerció en vivo.** Envuelve la lista como `{"object": "list", "data": [...]}` y mapea `conversation_id` → `id`. La superficie nativa `/grok/conversations` (rename, delete, share, messages) sigue intacta y es más rica. |
